...
$username = $_SESSION['name'];  // セッションから username を取得

// username から user_id を取得
$sql = "SELECT id FROM users WHERE name = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $username);  // プレースホルダーに username をバインド
$stmt->execute();
    ...
    <script>
        ...
        if (healthData.error) {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                let entry = healthData.find(row => row.date === dateStr);  // 'days' から 'date' に変更
                if (entry) {
